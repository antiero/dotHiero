# spreadSheetScroll.py - Adds 'Show in Spreadsheet' action for showing a Shot in the Spreadsheet TreeView
# By default this will make the Spreadsheet scroll automatically when a shot selection changes.
# You can turn off this selection change behaviour by setting gEnabledOnSelectionChange to False
# Working with some Caveats:
# -Does not work if Spreadsheet view is not visible for the active timeline
# -Does not work when text filtering is set in the Spreadsheet search box.
# A. Nasce, 6th July 2014.
...
gEnabledOnSelectionChange = True
...
class SpreadsheetScrollToAction(QAction):
  ...
  def showShotSelectionInSpreadsheet(self, *args):
    ...
    if len(spreadsheetTreeView)==0:
      print "Could not find a Spreadsheet view open with this name: %s!" % sequenceName
      return
    else:
