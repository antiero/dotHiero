...
class CustomSpreadsheetColumns(QtCore.QObject):
  ...
  standardColourSpaces = ['linear', 'sRGB', 'rec709', 'Cineon','Gamma1.8', 'Gamma2.2','Panalog', 'REDLog','ViperLog']
  arriColourSpaces = ['Video - Rec709','LogC - Camera Native','Video - P3','ACES','LogC - Film','LogC - Wide Gamut']
  r3dColourSpaces = ['Linear','Rec709','REDspace','REDlog','PDlog685','PDlog985','CustomPDlog','REDgamma','SRGB','REDlogFilm','REDgamma2','REDgamma3']
  ...
  def getIcon(self, row, column, item):
    ...
    if currentColumn['name'] == 'Shot Status':
      status = item.status()
      if status:
        return QtGui.QIcon(gStatusTags[status])
...
# Menu which adds a Set Status Menu to Timeline and Spreadsheet Views
class AssignArtistMenu(QtGui.QMenu):

  def __init__(self):
      QtGui.QMenu.__init__(self, "Assign Artist", None)
