...
class NodeModel(QtCore.QAbstractListModel):
  def __init__(self, mlist, weights, num_items = 5, filtertext = ""):
...
class TabTabTabWidget(QtGui.QDialog):
  ...
  def move_selection(self, where):
    if where not in ["first", "up", "down"]:
       raise ValueError("where should be either 'first', 'up', 'down', not %r" % (where))
  ...
  def create(self):
    ...
    msgBox.setText("Creating a thing: %s" % str(thing))
    ret = msgBox.exec_()  
...
import hiero.ui
action = QtGui.QAction("Node Selector", None)
action.setShortcut(QtGui.QKeySequence(Qt.Key_Tab))
action.triggered.connect(showTabMenu)
